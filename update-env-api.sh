...
# سكريبت تحديث متغيرات البيئة في Render باستخدام API
echo "🚀 تحديث متغيرات البيئة في Render باستخدام API..."

# الحصول على API Token من Render CLI
echo "🔑 الحصول على API Token..."
API_TOKEN=$(render auth token 2>/dev/null)

if [ -z "$API_TOKEN" ]; then
    echo "❌ لم يتم العثور على API Token"
    echo "🔐 يرجى تسجيل الدخول أولاً باستخدام: render login"
    exit 1
fi

echo "✅ تم الحصول على API Token"
...
SERVICE_ID=$(curl -s -H "Authorization: Bearer $API_TOKEN" \
...
if echo "$RESTART_RESPONSE" | jq -e '.service' > /dev/null 2>&1; then
    ...
    echo "📄 الاستجابة: $RESTART_RESPONSE"
fi
